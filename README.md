...
dependencies, a vendored dependency tree, and notes.
...
`vendor/` contains the full source of all 383 transitive dependencies, and
`.cargo/config.toml` points cargo at it instead of crates.io. No build step
touches the network. Full rustdoc for the whole dependency tree is pre-built at
`target/doc/` (`cargo doc --open`).
...
| [04-offline-workflow.md](docs/04-offline-workflow.md) | What works offline, what does not, how to regenerate `vendor/`. |
...
.cargo/config.toml   vendored-source + offline configuration
vendor/              565 MB of dependency source
src/main.rs          empty
